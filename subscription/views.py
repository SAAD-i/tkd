...
class SubscriptionPlanView(APIView):
    ...
    def post(self, request):
        plan = PlanSerializer(data=request.data, many=False)
        if plan.is_valid():
            plan.save()
...
class SubscriptionView(APIView):
    ...
    def post(self, request):
        try:
            ...
            user.is_pro = True
            user.save()
            serializer = SubscriptionSerializer(user, many=False)
            return Response(serializer.data, status.HTTP_200_OK)
        except:
            return Response(serializer.errors, status.HTTP_400_BAD_REQUEST)
        
    def delete(self, request, id):
        subscription = get_object_or_404(SubscriptionPlan, id=id)
        if subscription:
            subscription.delete()
    ...
    def put(self, request, id):
        subscription = get_object_or_404(SubscriptionPlan, id=id)
        if subscription:
            subscription.plan = request.data['plan']
